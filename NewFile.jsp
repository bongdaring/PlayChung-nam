...
<%
	...
	ArrayList<foodEntity> users = new ArrayList<foodEntity>();
	users = new foodAction().getSearchList(search);
%>
<meta charset="UTF-8">
...
<script>
	...
	//지도에 표시할 원을 생성합니다
	geocoder.addressSearch('<%=users.get(1).getAddress()%>', function(result, status) {
...
<table>
	...
	%>
	<tr>
		<td><%=review.getWhere()%></td>
		<td><%=review.getName()%></td>
		<td id="adress"><%=review.getAddress()%></td>
		<td><%=review.getPhone()%></td>
		<td><%=review.getMenu()%></td>
		<td>
			<a onclick="return confirm('추가하시겠습니까?')"
				href="./addfood.jsp?name=<%=review.getName()%>"
			>추가</a>
		</td>
		<td>
			<a onclick="return confirm('지도보기')"
				href="./usercheck.jsp?address=<%=review.getAddress()%>"
			>지도보기</a>
		</td>
